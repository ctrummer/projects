...
public class TestStarter {
    ...
    private final static String classpath = "./:" + "./bin/:" + // baseDir + ";" + baseDir + "bin/;"
                                                                // +
                                                                // //
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        for (int listenerCounter = 1; listenerCounter <= Configuration.numberOfSubsribers; listenerCounter++) {
            if (listenerCounter != Configuration.numberOfSubsribers) {
                createSubVm("java", "-cp", classpath, "Listener",
                        Configuration.destination_listener, "Listener" + listenerCounter, "all");
            } else {
                createSubVm("java", "-cp", classpath, "Listener",
                        Configuration.destination_listener, "Listener" + listenerCounter, "1");
            }
        ...
        for (int publisherCounter = 1; publisherCounter <= Configuration.numberOfPublishers; publisherCounter++) {
            createSubVm("java", "-cp", classpath, "ListenerPublisher", "" + publisherCounter);
            System.out.println("Publisher number " + publisherCounter + " started.");
        }
